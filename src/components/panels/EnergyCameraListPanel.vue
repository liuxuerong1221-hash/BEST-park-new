<template>
  <BasePanel class="energy-camera-list" title="摄像头列表">
    <!-- 搜索 + 筛选 -->
    <div class="energy-camera-list__toolbar">
    ...
    <!-- 表格 -->
    <div class="energy-camera-list__table">
      ...
      <div class="energy-camera-list__body">
        <div
          v-for="c in filtered"
          :key="c.id"
          class="meter-row meter-row--selectable"
          :class="{ 'meter-row--selected': c.id === selectedId }"
          role="button"
          tabindex="0"
        ...
        >
          <span class="meter-row__name">{{ c.name }}</span>
          <span class="meter-row__location">{{ c.location }}</span>
          <span class="meter-row__status">
            <span class="status-badge" :class="`status-badge--${c.status}`">
    ...
    <!-- 分页器 -->
    <div class="energy-camera-list__pagination">
      <button
...
function statusLabel(s: CameraStatus) { return STATUS_TEXT[s] }

const keyword = ref('')

const filterOpen = ref(false)
const filter = ref<'all' | CameraStatus>('all')
const filterOptions = [
...
function selectFilter(v: 'all' | CameraStatus) {
  filter.value = v
...
const filteredAll = computed(() => {
  ...
  if (props.filterBuildingId != null) {
    list = list.filter(c => c.buildingId === props.filterBuildingId)
  }
  if (keyword.value.trim()) {
    const k = keyword.value.trim()
    list = list.filter(c => c.name.includes(k) || c.location.includes(k))
  }
  if (filter.value !== 'all') {
