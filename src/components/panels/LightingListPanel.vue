<template>
  <BasePanel class="lighting-list" title="照明回路列表">
    <!-- 搜索 + 筛选 -->
    <div class="lighting-list__toolbar">
    ...
    <!-- 表格 -->
    <div class="lighting-list__table">
      <div class="meter-row meter-row--header">
        ...
        <span class="meter-row__power">开关</span>
      </div>

      <div class="lighting-list__body">
        <div
          v-for="m in filtered"
          :key="m.id"
        ...
        >
          ...
          <span class="meter-row__power">
            <span class="power-badge" :class="`power-badge--${powerTone(m)}`">
              {{ powerLabel(m) }}
            </span>
          </span>
        </div>
    ...
    <!-- 分页器 -->
    <div class="lighting-list__pagination">
      <button
...
const STATUS_TEXT: Record<LightingStatus, string> = {
  'online': '在线',
  'offline': '离线',
}
function statusLabel(s: LightingStatus) { return STATUS_TEXT[s] }

function powerLabel(m: LightingDevice) {
  if (m.status === 'offline') return '—'
  return m.powerState === 'on' ? '已开启' : '已关闭'
}

function powerTone(m: LightingDevice): 'on' | 'off' | 'na' {
  if (m.status === 'offline') return 'na'
  return m.powerState
}
...
const filter = ref<'all' | LightingStatus>('all')
const filterOptions = [
...
function selectFilter(v: 'all' | LightingStatus) {
  filter.value = v
...
.meter-row {
  display: grid;
  grid-template-columns: 1.4fr 1.3fr 0.9fr 0.9fr;
  align-items: center;
  height: 40px;
  padding: 0 12px;
  font-size: $font-size-xs;
...
.power-badge {
  ...
  &--na {
    color: #5C6E80;
    background: transparent;
    border-color: rgba(92, 110, 128, 0.4);
  }
}
